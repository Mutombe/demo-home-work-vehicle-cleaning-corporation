...
function HeroSection() {
  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-teal-950">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-teal-950/60 via-teal-950/30 to-teal-950/80 z-[1]" />
      </motion.div>
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.3 }}
          className="w-16 h-[2px] bg-teal-400 mb-6 origin-left" />
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          className="text-teal-300 text-xs font-semibold uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero.badge}
        </motion.p>
        <div className="overflow-hidden">
          <motion.h1 initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5, ease: [0.16, 1, 0.3, 1] }}
            ...
            INSPECT. TREAT.<br /><span className="text-teal-400 italic font-bold">PREVENT. MONITOR.</span>
          </motion.h1>
        </div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2 }} className="flex flex-wrap gap-4 mt-10">
          <Link to="/contact" className="group inline-flex items-center gap-3 bg-teal-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-teal-600 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <Link to="/services" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-teal-400/50 hover:text-teal-300 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaSecondary}
          </Link>
        </motion.div>
      </motion.div>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2 }}>
          <ArrowDown size={14} className="text-teal-400/40" />
        </motion.div>
      </motion.div>
    </section>
  );
      ...
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-teal-500 font-heading text-lg sm:text-2xl italic tracking-wider">{item}</span>
            <span className="text-teal-500/30 text-sm">&diams;</span>
          </span>
        ))}
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-teal-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Services</p>
            ...
            <Link to="/services" className="group text-gray-400 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-teal-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-teal-500/15 font-heading text-6xl sm:text-7xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 border border-teal-500/30 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm group-hover:bg-teal-500/20 group-hover:border-teal-500/60 transition-all duration-500">
                    <IconComp size={18} weight="fill" className="text-teal-500" />
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-10">
                    ...
                    <div className="flex items-center gap-2 mt-3 text-teal-500 group-hover:translate-x-1 transition-transform duration-300">
                      <span className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Explore</span>
                      <ArrowRight size={14} />
                    </div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-teal-500 to-teal-300 z-10" />
                </Link>
              </motion.div>
            );
          }}
...
function ProcessSteps() {
  ...
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const steps = [
    { num: '01', title: 'Inspect', desc: 'Thorough assessment of your property to identify pest types, entry points, and infestation levels.' },
    { num: '02', title: 'Treat', desc: 'Targeted treatment using approved, eco-safe products tailored to your specific pest problem.' },
    { num: '03', title: 'Prevent', desc: 'Seal entry points, install barriers, and implement preventive measures to stop re-infestation.' },
    { num: '04', title: 'Monitor', desc: 'Regular follow-up visits and monitoring to ensure your property stays pest-free long-term.' },
  ];
  return (
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-14">
          ...
          <h2 className="font-heading text-gray-900 leading-[0.95] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
            Our 4-Step <span className="text-teal-500">Process</span>
          </h2>
        </motion.div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, i) => (
            <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: i * 0.1 }} className="relative p-6 border border-gray-200 hover:border-teal-500/50 transition-colors group">
              <span className="text-teal-500/10 font-heading text-6xl italic leading-none absolute top-4 right-4">{step.num}</span>
              <div className="w-10 h-10 bg-teal-500 flex items-center justify-center mb-4">
                <span className="text-white text-sm font-bold" style={{ fontFamily: 'var(--font-sans)' }}>{step.num}</span>
              </div>
              <h3 className="font-heading text-gray-900 text-lg italic mb-2">{step.title}</h3>
              <p className="text-gray-500 text-sm leading-relaxed" style={{ fontFamily: 'var(--font-sans)' }}>{step.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
    ...
    <section ref={ref} className="bg-gray-50 py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-teal-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Recent Work</p>
              <h2 className="font-heading text-gray-900 leading-[0.92] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                Featured <span className="text-teal-500">Projects</span>
              </h2>
            </div>
            <Link to="/projects" className="group text-gray-400 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-teal-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          {projects.items.slice(0, 6).map((project, i) => (
            <motion.div key={project.slug} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className={`overflow-hidden relative ${i === 0 ? 'aspect-square sm:aspect-auto sm:h-full' : 'aspect-[4/3]'}`}>
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-teal-500/30 transition-colors duration-500 z-10" />

                {/* Category badge */}
                <div className="absolute top-4 left-4 z-10">
                  <span className="bg-teal-500/90 text-white text-[10px] uppercase tracking-[0.2em] px-3 py-1.5 font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>
                    {project.category}
                  </span>
                </div>
                ...
                {/* Number watermark */}
                <div className="absolute top-4 right-5 z-10">
                  <span className="text-teal-500/10 font-heading text-5xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                </div>
                ...
                {/* Bottom accent */}
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-teal-500 z-10 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <section ref={containerRef} className="bg-white py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div>
            <div className="w-12 h-[2px] bg-teal-500 mb-6" />
            <p className="text-teal-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Selected Works</p>
          ...
          {projects.items.map((project, i) => (
            <div key={project.slug} className="group relative flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[420px] overflow-hidden">
              <div className="aspect-[3/4] overflow-hidden relative">
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-teal-500/40 transition-colors duration-500 z-10" />
  ...
  return (
    <section ref={ref} className="relative bg-white overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-48 h-48 bg-teal-500/5 rounded-full blur-3xl" />
        <div className="absolute top-1/2 right-1/4 -translate-y-1/2 w-48 h-48 bg-teal-500/5 rounded-full blur-3xl" />
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="font-heading text-teal-500 leading-none italic" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)' }}>
              ...
              {i < stats.length - 1 && (
                <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-teal-500/15 to-transparent" />
              )}
            </motion.div>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-14">
          ...
          <h2 className="font-heading text-gray-900 leading-[0.95] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
            The <span className="text-teal-500">Difference</span> We Make
          </h2>
        </motion.div>
        <div className="grid sm:grid-cols-2 gap-6">
          {siteData.projects.items.slice(0, 4).map((project, i) => (
            <motion.div key={project.slug} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: i * 0.1 }} className="relative overflow-hidden aspect-[16/10] group">
              <img src={project.image} alt={project.title} className="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-700" loading="lazy" />
              <div className="absolute inset-0 bg-gradient-to-t from-gray-900/80 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-5 z-10">
                <span className="text-teal-500 text-[10px] font-bold uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>{project.category}</span>
                <h4 className="text-white font-heading text-lg italic mt-1">{project.title}</h4>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-teal-500 mb-6" />
            <p className="text-teal-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Story</p>
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Home,<br /><span className="text-teal-500">Story</span>
            </h2>
            {about.story.map((p, i) => (
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-teal-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.established}</div>
                <div className="text-gray-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Founded</div>
              </div>
              <div>
                <div className="text-teal-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.projectsCompleted}</div>
                <div className="text-gray-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Completed</div>
              </div>
              <div>
                <div className="text-teal-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.yearsExperience}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            ...
            <div className="absolute -bottom-3 -right-3 w-16 h-16 border-b-2 border-r-2 border-teal-500/30" />
          </motion.div>
        </div>
      </div>
    </section>
    ...
    <section ref={ref} className="bg-white py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="w-12 h-[2px] bg-teal-500 mx-auto mb-6" />
          <p className="text-teal-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>What Drives Us</p>
          <h2 className="font-heading text-gray-900 leading-[0.95] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {about.values.map((value, i) => (
            <motion.div key={value.title} initial={{ opacity: 0, y: 25 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              className="relative p-6 sm:p-8 border border-gray-200 hover:border-teal-500/40 transition-all duration-500 group">

              {/* Number watermark */}
              <span className="absolute top-4 right-4 text-teal-500/8 font-heading text-6xl italic leading-none">
                {String(i + 1).padStart(2, '0')}
              </span>

              {/* Icon badge */}
              <div className="w-12 h-12 bg-teal-500/10 flex items-center justify-center mb-5 group-hover:bg-teal-500/20 transition-colors duration-500">
                <CheckCircle size={22} weight="fill" className="text-teal-500" />
              </div>
              ...
              {/* Bottom accent line */}
              <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-teal-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="absolute -top-3 -left-3 w-20 h-20 border-t-2 border-l-2 border-teal-500/40" />
            <div className="absolute -bottom-3 -right-3 w-20 h-20 border-b-2 border-r-2 border-teal-500/40" />
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.9, delay: 0.15 }}>
            <div className="w-12 h-[2px] bg-teal-500 mb-6" />
            <p className="text-teal-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Difference</p>
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-teal-500">Home,</span>
            </h2>
              ...
              {points.map((point, i) => (
                <motion.div key={point.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                  ...
                  <div className="shrink-0 mt-1">
                    <div className="w-8 h-8 border border-teal-500/30 flex items-center justify-center bg-teal-500/5">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                {t.avatar && <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover object-center border-2 border-teal-500/30" loading="lazy" />}
                ...
                <div className="flex items-center gap-0.5 mt-1">
                  {[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-teal-500" />)}
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-gray-200 flex items-center justify-center text-gray-400 hover:text-teal-500 hover:border-teal-500/30 transition-colors" aria-label="Previous">
              <CaretLeft size={16} />
            </button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-teal-500' : 'w-3 bg-gray-200'}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-gray-200 flex items-center justify-center text-gray-400 hover:text-teal-500 hover:border-teal-500/30 transition-colors" aria-label="Next">
              <CaretRight size={16} />
            </button>
          </div>
        </motion.div>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={siteData.hero.backgroundImages[0]?.url} alt="Home, Work & Vehicle Cleaning Corporation" className="w-full h-[130%] object-cover object-center" loading="lazy" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          <div className="w-16 h-[2px] bg-teal-500 mx-auto mb-8" />
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            READY TO<br /><span className="text-teal-500">GET STARTED?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-teal-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:shadow-xl transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          <div className="mt-16 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-center gap-6 text-xs text-white/25 uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>
            <a href={`tel:${business.phoneRaw}`} className="flex items-center gap-2 hover:text-teal-500 transition-colors"><Phone size={14} /> {business.phone}</a>
